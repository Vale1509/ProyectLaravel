    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('voluntario.create') }}" method="post">
                    @csrf
                    <div class="mb-3">
        ...
        <table class="table table-striped table-bordered table-hover">
            ...
            <tbody class="table-group-divider">
                @foreach ($datos as $item)
                    <tr>
                        <th>{{ $item->idvoluntarios }}</th>
                        <td>{{ $item->nombre }}</td>
                        <td>{{ $item->dui}}</td>
                        <td>{{ $item->telefono }}</td>
                        <td>{{ $item->correo }}</td>
                        <td>
                            <a href="" data-bs-toggle="modal" data-bs-target="#modalModificar{{$item->idvoluntarios}}"
                                class="btn btn-warning btn-sm"> <i class="fa-solid fa-pen-to-square"></i></a>
                            <a href="{{route('voluntario.delete',$item->idvoluntarios)}}" onclick="return res()" class="btn btn-danger btn-sm"> <i class="fa-solid fa-trash"></i></a>
                        </td>

                        <!-- Modal Modificar-->
                        <div class="modal fade" id="modalModificar{{$item->idvoluntarios}}" tabindex="-1" aria-labelledby="exampleModalLabel"
                            aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                        <form action="{{route('voluntario.update')}}" method="post">
                                            @csrf
                                            <div class="mb-3">
                                                <div>
                                                    <label for="id" class="form-label">ID</label>
                                                    <input type="text" class="form-control" id="id"
                                                        name="id" value="{{$item->idvoluntarios}}" readonly>
                                                    </div>
                                                <div>
                                                <label for="nombre" class="form-label">Nombre</label>
