...
<body>
    ...
    @section('contenido')
    <h1 class="text-center p-3">Adoptantes</h1>
    @if(session("correcto"))
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="modalRegistar">Registrar Adoptante</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="{{ route('adoptante.create') }}" method="post">
                    @csrf
                    <div class="mb-3">
        ...
        <table class="table table-striped table-bordered table-hover">
            ...
            <tbody class="table-group-divider">
                @foreach ($datos as $item)
                    <tr>
                        <th>{{ $item->idadoptantes }}</th>
                        <td>{{ $item->nombre }}</td>
                        <td>{{ $item->dui}}</td>
                        <td>{{ $item->telefono }}</td>
                        <td>{{ $item->correo }}</td>
                        <td>
                            <a href="" data-bs-toggle="modal" data-bs-target="#modalModificar{{$item->idadoptantes}}"
                                class="btn btn-warning btn-sm"> <i class="fa-solid fa-pen-to-square"></i></a>
                            <a href="{{route('adoptante.delete',$item->idadoptantes)}}" onclick="return res()" class="btn btn-danger btn-sm"> <i class="fa-solid fa-trash"></i></a>
                        </td>

                        <!-- Modal Modificar-->
                        <div class="modal fade" id="modalModificar{{$item->idadoptantes}}" tabindex="-1" aria-labelledby="exampleModalLabel"
                            aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                        <form action="{{route('adoptante.update')}}" method="post">
                                            @csrf
                                            <div class="mb-3">
                                                <div>
                                                    <label for="id" class="form-label">ID</label>
                                                    <input type="text" class="form-control" id="id"
                                                        name="id" value="{{$item->idadoptantes}}" readonly>
                                                    </div>
                                                <div>
                                                <label for="nombre" class="form-label">Nombre</label>
